...
void Maps :: Generate—oordinatesOfMaps()
{
	srand(time(NULL));
...
//void Maps :: InstallTheInitialMap(Player*& player) // ÔÂÂ‰ÂÎ‡Ú¸ ÔÓÒÎÂ „ÂÌÂ‡ˆËË ÔÂÒÓÌ‡Ê‡
//{
...
{
	if (!current_map->—heckPersonLocationOnMap(player->get_player().getPosition().x + 40, player->get_player().getPosition().y + 90))
	{
		for (int i = 0; i < maps.size(); i++)
		{
			if (current_map != maps[i])
			{
				if (maps[i]->—heckPersonLocationOnMap(player->get_player().getPosition().x + 40, player->get_player().getPosition().y + 90))
				{
